...
class EchartsAirMap extends React.Component {
            ...
            for (var i = 0; i < toolTipData.length; i++) {
              if (params.name == toolTipData[i].name) {
                ...
                for (var j = 0; j < toolTipData[i].value.length; j++) {
                  toolTiphtml +=
                    toolTipData[i].value[j].name + ':' + toolTipData[i].value[j].value + '<br>';
                }
              }
            }
  ...
  render() {
    ...
    return (
      <div>
        <div id={id} style={{ width: '100%', height: height ? height : '800px' }} />
      </div>
    );
  }
}

export default EchartsAirMap;
